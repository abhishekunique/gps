...
EXP_DIR = BASE_DIR + '/../experiments/reach_tendonjoint_invariant/'
...
common = {
    ...
    'policy_opt': {
        ...
        },
        {
            'dim_hidden': [10],
            'num_filters': [10, 20],
            'obs_include': [JOINT_ANGLES, JOINT_VELOCITIES, TENDON_LENGTHS, TENDON_VELOCITIES,END_EFFECTOR_POINTS, END_EFFECTOR_POINT_VELOCITIES],
            'obs_vector_data': [JOINT_ANGLES, JOINT_VELOCITIES,  TENDON_LENGTHS, TENDON_VELOCITIES, END_EFFECTOR_POINTS, END_EFFECTOR_POINT_VELOCITIES],
            'obs_image_data':[],
            'image_width': IMAGE_WIDTH,
            'image_height': IMAGE_HEIGHT,
            'image_channels': IMAGE_CHANNELS,
...
all_offsets = [[np.asarray([0.5, 0., -1.15])],
               [np.asarray([0.75 ,0, 0.6])],
               [np.asarray([0.6, 0.0, 1.1])],
               [np.asarray([0.75 ,0, -0.6])],
               [np.asarray([0.9, 0., -0.8])],
               [np.asarray([0.9 ,0, 0.8])],
               [np.asarray([0.4, 0.0, 1.2])],
               [np.asarray([1.2, 0., -0.8])]]
agent = [{
            'type': AgentMuJoCo,
            'filename': './mjc_models/finger_weight_torque.xml',
            'x0': np.zeros(6),
            'dt': 0.05,
            'substeps': 5,
        ...
        },
        {
            ...
            'x0': np.concatenate([np.zeros(6), np.ones((3,)), np.zeros((3,))]),
            'dt': 0.05,
            'substeps': 5,
            'pos_body_offset': all_offsets,
            'pos_body_idx': np.array([6]),
...
fk_cost_2 = [{
    'type': CostFK,
    'target_end_effector': np.concatenate([agent[1]['pos_body_offset'][i][0], np.array([0., 0., 0.])]),
    'wp': np.array([1, 1, 1, 0, 0, 0]),
    'l1': 0.1,
    'l2': 10.0,
    'alpha': 1e-5,
