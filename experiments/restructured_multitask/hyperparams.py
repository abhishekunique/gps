...
from gps.algorithm.policy_opt.tf_model_example_multirobot import example_tf_network_multi, multitask_multirobot_fc_hardcoded
...
EXP_DIR = BASE_DIR + '/../experiments/multitask/'
...
common = {
    ...
    'policy_opt': {
        'type': PolicyOptTf,
        'network_model': multitask_multirobot_fc_hardcoded,
        'network_params': [{
            'dim_hidden': [10],
            'num_filters': [10, 20],
...
algorithm[2]['cost'] = [{
    'type': CostSum,
    'costs': [fk_cost_2[i], fk_cost_2_gripper[i], torque_cost_2[i]],
    'weights': [1.,0.5, 0.5],
} for i in agent[2]['train_conditions']]
...
fk_cost_3 = [{
    'type': CostFK,
    'target_end_effector': np.concatenate([np.array([0,0,0]), 
                                           np.array([0.05, 0.05, 0.05]) + agent[3]['pos_body_offset'][i][1],
                                           np.array([0,0,0])]),
    'wp': np.array([0, 0, 0, 1, 1, 1,0,0,0]),
    'l1': 0.1,
    'l2': 10.0,
...
algorithm[3]['cost'] = [{
    'type': CostSum,
    'costs': [fk_cost_3[i], torque_cost_3[i]],
    'weights': [1.0, 0.5],
