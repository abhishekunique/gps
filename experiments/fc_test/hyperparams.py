...
#from gps.algorithm.policy_opt.tf_model_example_multirobot import example_tf_network_multi_contrastive
...
EXP_DIR = BASE_DIR + '/../experiments/fc_test/'
...
common = {
    ...
    'policy_opt': {
        ...
        'network_params': [{
            ...
            # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[1][s] for s in OBS_INCLUDE]),
            'batch_size': 25,
        }],
        'iterations': 2000,
...
algorithm[0]['cost'] = [{
    'type': CostSum,
    'costs': [fkblock_cost_1[i], fk_cost_1[i], torque_cost_1[i]],
    'weights': [0.7, 1.0, 0.5],
} for i in common['train_conditions']]

algorithm[1]['cost'] = [{
    'type': CostSum,
    'costs': [fkblock_cost_1[i], fk_cost_2[i], torque_cost_2[i]],
    'weights': [0.7, 1.0, 0.5],
} for i in common['train_conditions']]
