...
EXP_DIR = BASE_DIR + '/../experiments/peginsert_invariant/'
...
fk_cost_0 = [{
    'type': CostFK,
    'target_end_effector': np.concatenate([np.array([0., 0.0, 0.])+ agent[0]['pos_body_offset'][i][0], np.array([0., 0., 0.])]),
...
algorithm[0]['cost'] = [{
    'type': CostSum,
    'costs': [fk_cost_0[i], torque_cost_0[i]],
    'weights': [1.0,1.0],
} for i in agent[0]['train_conditions']]
...
fk_cost_1 = [{
    'type': CostFK,
    'target_end_effector': np.concatenate([np.array([0., 0.0, 0.])+ agent[1]['pos_body_offset'][i][0], np.array([0., 0., 0.])]),
    'wp': np.array([1, 1, 1, 0, 0, 0]),
    'l1': 0.1,
    'l2': 10.0,
    'alpha': 1e-5,
