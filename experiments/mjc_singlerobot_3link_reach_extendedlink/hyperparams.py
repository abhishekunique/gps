...
from gps.algorithm.cost.cost_state_dev import CostStateDev
...
from gps.algorithm.policy_opt.tf_model_example_multirobot import multi_input_multi_output_images_shared_fc
...
EXP_DIR = BASE_DIR + '/../experiments/mjc_singlerobot_3link_3link_reach_extendedlink/'
...
common = {
    ...
    'policy_opt': {
        'type': PolicyOptTf,
        'network_model': multi_input_multi_output_images_shared_fc,
        'network_params': [{
            'dim_hidden': [10],
            'num_filters': [10, 20],
        ...
        'restore_all_wts': '/home/coline/abhishek_gps/gps/3_link_weights_533.npy'
...
agent = [{
    'type': AgentMuJoCo,
    'filename': './mjc_models/arm_3link_extended.xml',
    'x0': np.zeros(6),
    'dt': 0.05,
    'substeps': 5,
    'pos_body_offset': all_offsets,
    'pos_body_idx': np.array([6]),
    # 'noisy_body_var': np.ones(3),
    # 'noisy_body_idx': [np.array([2,3,4]) for c in range(8)],
...
# {
#     'type': AgentMuJoCo,
#     'filename': './mjc_models/arm_3linkdamped_reach.xml',
#     'x0': np.zeros(6),
#     'dt': 0.05,
#     'substeps': 5,
#     'pos_body_offset': all_offsets,
...
# INIT_POLICY_DIR = "/home/coline/abhishek_gps/gps/experiments/mjc_singlerobot_3link_reach_multicondition/data_files/"

# algorithm[0]['init_traj_distr'] = {
#     'type': init_from_file,
#     'filename': [INIT_POLICY_DIR + ('cond%d_dist.pkl' % i)
#             for i in range(4)]
# }
...
algorithm[0]['cost'] = [{
    'type': CostSum,
    'costs': [torque_cost_1[i], fk_cost_1[i]],# dev_cost[i]],
    'weights': [1.0, 1.0],# 0.4],
} for i in common['train_conditions']]
