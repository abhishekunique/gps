...
class GPSMain(object):
    ...
    def run(self, itr_load=None):
        ...
        for itr in range(itr_start, self._hyperparams['iterations']):
            ...
            for robot_number in range(self.num_robots):
                ...
                self.agent[robot_number].clear_samples()
            out = self.dump_traj_sample_lists(traj_sample_lists)
            print "accuracy", np.mean(out[out.keys()[0]])
            for robot_number in range(self.num_robots):
    ...
    def run_badmm(self, testing, load_old_weights, itr_load=None):
        ...
        self.check_itr = 8
        import IPython
        for i in range(len(self.algorithm[0].cur)):
            self.algorithm[0].cur[i].traj_distr.chol_pol_covar*=1
        ...
        for itr in range(itr_start, self._hyperparams['iterations']):
            ...
            out = self.dump_traj_sample_lists(traj_sample_lists)
            print "mean traj", np.mean(out[out.keys()[0]])
            time3 = time.clock()
            ...
            self._take_iteration_start(itr, traj_sample_lists[robot_number])
            ...
            pol_samples = {}
            for robot_number in range(self.num_robots):
                self.algorithm[robot_number].end_iteration()
                ...
                pol_samples[robot_number] = pol_sample_lists
                # if self.agent[robot_number].nan_flag:
                #     IPython.embed()
                self._log_data(itr, traj_sample_lists[robot_number], pol_sample_lists, robot_number=robot_number)
            time6 = time.clock()
    ...
    def _take_iteration_start(self, itr, sample_lists, robot_number=0):
        ...
        if itr ==0:
            self._take_iteration_shared()
        for robot_number in range(self.num_robots):
            self.algorithm[robot_number].iteration_middle(sample_lists, itr)
    ...
    def _take_iteration_shared(self):
        ...
        for robot_number in range(self.num_robots):
            ...
            obs, tgt_mu, tgt_prc, tgt_wt = self.algorithm[robot_number]._update_policy()
...
def main():
    ...
    if args.targetsetup:
        ...
        seed = hyperparams.config.get('random_seed', 453)
        random.seed(seed)
        np.random.seed(seed)
        ...
        seed = hyperparams.config.get('random_seed', 83)
        random.seed(seed)
        np.random.seed(seed)
