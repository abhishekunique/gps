...
def peg_right_4link(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 4,
        JOINT_VELOCITIES: 4,
    ...
    agent_dict['network_params']= {
        ...
        'task_specific_idx': range(8,11)+range(11,14)+range(14,17)+range(17,20),
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/4link_peg_right.xml',
        'x0': np.zeros(8),
        'dt': 0.05,
        'substeps': 5,
    ...
    fk_cost_0 = [{
        'type': CostFK,
        'target_end_effector': np.concatenate([np.array([0.0, 0.0, 0.0])+ agent_dict['agent']['pos_body_offset'][i][0], np.array([0., 0., 0.])]),
