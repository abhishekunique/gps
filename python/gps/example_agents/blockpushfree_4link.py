...
def blockpushfree_4link(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 7,
        JOINT_VELOCITIES: 7,
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(5)+range(7,12)+range(14,20)+range(26,32),
        'task_specific_idx': range(5,7)+ range(12,14)+range(14,20)+range(20,26)+range(26,32)+range(32,38),
        'dim_output':5,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/4link_gripper_push_2step.xml',
        'x0': np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, 0., 0., 0.]), np.zeros((7,))]),
        'dt': 0.05,
        'substeps': 5,
    ...
    cost_tgt = np.zeros((7,))
    cost_wt = np.array([0, 0, 0, 0, 1, 0, 0])
    state_cost = [{
        'type': CostState,
        'l1': 0.0,
        'l2': 10.0,
