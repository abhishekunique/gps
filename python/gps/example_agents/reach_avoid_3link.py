...
def reach_avoid_3link(robot_number, num_robots):
    all_offsets = [
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(9)+range(15,18),
        'task_specific_idx': range(9,15)+range(18, 24),
        'dim_output':4,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/3link_reach_avoid.xml',
        'x0': np.zeros(8),
        'dt': 0.05,
        'substeps': 5,
