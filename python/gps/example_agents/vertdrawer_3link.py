...
def vertdrawer_3link(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 4,
        JOINT_VELOCITIES: 4,
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(3)+range(4,7)+range(8,14)+range(20,26),
        'task_specific_idx': range(3,4)+ range(7,8)+range(8,14)+range(14,20)+range(20,26)+range(26,32),
        'dim_output':3,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/3link_drawer_vertical.xml',
        'x0': np.concatenate([np.array([0, 0.0, 0.0, 0.0]), np.zeros((4,))]),
        'dt': 0.05,
        'substeps': 5,
