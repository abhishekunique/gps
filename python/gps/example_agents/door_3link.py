...
def door_3link(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 5,
        JOINT_VELOCITIES: 5,
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(4)+range(5,9)+range(10,16)+range(22,28),
        'task_specific_idx': range(4,5)+ range(9,10)+range(10,16)+range(16,22)+range(22,28)+range(28,34),
        'dim_output':4,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/3link_door.xml',
        'x0': [np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, 0.4]), np.zeros((5,))]), 
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, -0.4]), np.zeros((5,))]),
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, 0.2]), np.zeros((5,))]), 
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, -0.65]), np.zeros((5,))]),
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, 0.3]), np.zeros((5,))]), 
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, -0.3]), np.zeros((5,))]),
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, 0.8]), np.zeros((5,))]), 
                np.concatenate([np.array([np.pi/2, 0.0, 0.0, 0.0, -0.8]), np.zeros((5,))])],
        'dt': 0.05,
        'substeps': 5,
