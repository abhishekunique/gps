...
def push_4link_shortjoint(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 6,
        JOINT_VELOCITIES: 6,
    ...
    agent_dict['network_params']= {
        ...
        'task_specific_idx': range(12,15) + range(21,24),#range(15,21)+range(24,30),
        'dim_output':4,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/4link_gripper_push_shortjoint.xml',
        'x0': np.zeros(12),
        'dt': 0.05,
        'substeps': 5,
