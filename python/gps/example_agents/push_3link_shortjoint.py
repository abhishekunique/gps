...
def push_3link_shortjoint(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 5,
        JOINT_VELOCITIES: 5,
    ...
    agent_dict['network_params']= {
        ...
        'task_specific_idx':  range(3,5)+range(8,10)+range(10,28),#range(13,19)+range(22,28),
        'dim_output':3,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/3link_gripper_push_shortjoint.xml',
        'x0': np.zeros(10),
        'dt': 0.05,
        'substeps': 5,
