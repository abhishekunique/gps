...
def reach_4link_green(robot_number, num_robots):
    ...
    #                [np.array([0., 0., -1.2])],
    #                [np.array([0.4, 0., -0.9])]]
    all_offsets = [ [ np.asarray([0.45, 0., 0.45]), np.asarray([-0.3, 0., -1.65]), np.asarray([0.4, 0., -1.3]), np.asarray([-0.4, 0.0, 0.7])],
    ...
    fk_cost_0 = [{
        'type': CostFK,
        'target_end_effector': np.concatenate([np.array([0.8, 0.0, 0.5])+ agent_dict['agent']['pos_body_offset'][i][1], np.zeros(12)]),
