...
def blockpushfree_2step_3link(robot_number, num_robots):
    SENSOR_DIMS = {
        JOINT_ANGLES: 5,
        JOINT_VELOCITIES: 5,
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(3)+range(5,8)+range(10,16)+range(22,28),
        'task_specific_idx': range(3,5)+ range(8,10)+range(10,16)+range(16,22)+range(22,28)+range(28,34),
        'dim_output':3,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/3link_push_2step.xml',
