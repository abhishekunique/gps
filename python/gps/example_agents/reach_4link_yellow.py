...
def reach_4link_yellow(robot_number, num_robots):
    ...
    #                [np.array([0., 0., -1.2])],
    #                [np.array([0.4, 0., -0.9])]]
    all_offsets = [ [np.asarray([0.45, 0., 0.45]), np.asarray([0.4, 0., -1.3]), np.asarray([-0.3, 0., -1.65]), np.asarray([-0.4, 0.0, 0.7])],
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(11)+range(23,26),
        'task_specific_idx': range(8,11) + range(11,23) +range(23,26)+ range(26,38) ,
    ...
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/arm_4link_reach_colors.xml',
        'x0': np.zeros(8),
        'dt': 0.05,
        'substeps': 5,
    ...
    fk_cost_0 = [{
        'type': CostFK,
        'target_end_effector': np.concatenate([np.array([0.8, 0.0, 0.5])+ agent_dict['agent']['pos_body_offset'][i][2], np.zeros(12)]),
