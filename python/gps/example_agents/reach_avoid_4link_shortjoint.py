...
from gps.algorithm.cost.cost_utils import eval_sqrt_term, evallogl2term
...
def reach_avoid_4link_shortjoint(robot_number, num_robots):
    all_offsets = [
        [np.array([0., 0., -1.2]), np.asarray([0.7, 0., -1.4])],
        [np.array([0.4, 0., -1.6]), np.asarray([0.6, 0., -.9])],
        [np.asarray([0.2, 0., -0.5]), np.asarray([0.3, 0., 0.]), ],
        [np.asarray([0., 0., 0.4]), np.asarray([0.3, 0., 0.]), ],
        [np.asarray([0.2, 0.0, 0.4]), np.asarray([0.6, 0., -1.4])],
        [np.asarray([0.4, 0., -0.7]), np.asarray([0., 0., 0])],
        [np.asarray([.5, 0.0, 0.3]), np.asarray([0., 0., 0.4])],
        [np.asarray([.7, 0.0, -0.3]), np.asarray([0., 0., -1.2])],
    ...
    agent_dict['network_params']= {
        ...
        'batch_size': 25,
        'robot_specific_idx': range(11)+range(17,20),
        'task_specific_idx': range(11,17)+range(20, 26),
        'dim_output':4,
        # 'dim_input': reduce(operator.mul, [SENSOR_DIMS[0][s] for s in OBS_INCLUDE]),
    }
    agent_dict['agent'] = {
        'type': AgentMuJoCo,
        'filename': './mjc_models/4link_reach_avoid_shortjoint.xml',
        'x0': np.zeros(8),
        'dt': 0.05,
        'substeps': 5,
        'pos_body_offset': all_offsets,
        'pos_body_idx': np.array([7, 8]),
        'conditions': 8,
    ...
    fk_cost_0 = [{
        'type': CostFK,
        'target_end_effector': np.concatenate([np.array([0.8, 0.0, 0.5])+ agent_dict['agent']['pos_body_offset'][i][0], np.zeros(6)]),
    ...
    avoid_cost = [{
        'type': CostFK,
        'target_end_effector': np.concatenate([np.array([0.8, 0.0, .5])+ agent_dict['agent']['pos_body_offset'][i][1], np.zeros(6)]),
        'wp': np.array([1, 1, 1, 0, 0, 0,0,0,0]),
        'l1': 0.1,
        'l2': 10.0,
        'evalnorm': eval_sqrt_term, #
        'alpha': 1e-5,
    } for i in agent_dict['agent']['train_conditions']]
    ...
    agent_dict['algorithm']['cost'] = [{
        'type': CostSum,
        'costs': [torque_cost_0[i], fk_cost_0[i], avoid_cost[i]],
        'weights': [1.0, 2.0, -1.0],
    } for i in agent_dict['agent']['train_conditions']]
