...
class CostCCA(Cost):
    ...
    def eval(self, sample):
        ...
        x = np.concatenate([x[:, 6:9], x[:, 9:12]], axis=1)
        feat_forward = np.dot(x, self.x_weights)#self.cca.transform(x)
        ...
        for t in range(T):
            ...
            np.concatenate([np.arange(6,9), np.arange(9,12)])
            ls[t, 6:9] = grad_mult[0:3]
            ls[t, 9:12] = grad_mult[3:6]
