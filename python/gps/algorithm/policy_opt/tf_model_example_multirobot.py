...
def multi_input_multi_output_images_shared_dc(dim_input=[27, 27], dim_output=[7, 7], batch_size=25, network_config=None):
    ...
    """
    # List of indices for state (vector) data and image (tensor) data in observation.
    print 'making multi-input/output-network DOMAIN CONFUSION'
    ...
    wc2shared = get_xavier_weights([filter_size, filter_size, num_filters[0], num_filters[1]], (pool_size, pool_size), name='wc2rnshared') # 5x5 conv, 32 inputs, 64 outputs
    bc2shared = init_bias([num_filters[1]], name='bc2rnshared')
    w_dc = init_weights([num_filters[-1]*2, num_robots], name='dc_weight_shared')  # 5x5 conv, 32 inputs, 64 outputs
    b_dc = init_bias([num_robots], name='dc_bias_shared')
    dc_vars = [w_dc, b_dc]
    other_vars = [wc1shared, bc1shared, wc2shared, bc2shared]
    dc_loss = []
    for robot_number, robot_params in enumerate(network_config):
        n_layers = 3
        layer_size = 20
        ...
        dc_output = tf.matmul(full_feature_points, w_dc) + b_dc
        ...
        nnets.append(TfMap.init_from_lists([nn_input, action, precision], [fc_output], [loss],
                                           feature_points=full_feature_points))
        other_vars += weights_FC
        other_vars += biases_FC
    
    return nnets, fc_vars, last_conv_vars, dc_vars, dc_loss, other_vars
