...
def init_weights_shared(shape):
    weights = tf.get_variable("weights", shape,
        initializer=tf.random_normal_initializer())
    return weights
    # return tf.get_variable Variable(tf.random_normal(shape, stddev=0.01), name=name)


def init_bias_shared(shape):
...
def multi_input_multi_output(dim_input=[27, 27], dim_output=[7, 7], batch_size=25):
    """
    ...
    for input_size, output_size, robot_number in zip(dim_input, dim_output, range(num_robots)):
        ...
        nn_input, action, precision = get_input_layer(input_size, output_size)
