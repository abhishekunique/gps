...
def model_imbalanced_fully_connected(dim_input=[27, 27], dim_output=[7, 7], batch_size=25, network_config=None):
    """
    ...
    with tf.variable_scope("shared_wts"):
        for robot_number, robot_params in enumerate(network_config):
            ...
            print dim_hidden[1:]
            print n_layers-2
            fc_output, weights_FC, biases_FC = get_mlp_layers_shared(mlp_input, n_layers-2, dim_hidden[1:], robot_number=robot_number)
            ...
            inv += [w_output, w_input, b_output, b_input]
            indiv_net_vars.append(inv)
            indiv_fc_vars.append([None])
            fc_vars = None
    return nnets, fc_vars, last_conv_vars, all_vars, [], indiv_net_vars, indiv_fc_vars
