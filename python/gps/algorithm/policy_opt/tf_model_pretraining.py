...
def multi_input_multi_output_images_pretraining(dim_input=[27, 27], dim_output=[7, 7], batch_size=25, network_config=None):
    """
    ...
    with tf.variable_scope("shared_wts"):
        for robot_number, robot_params in enumerate(network_config):
            ...
            weights = {
                ...
                'pretrain_w': init_weights([conv_out_size, dim_hidden], name='pretrain_w'+str(robot_number)),
                'pretrain_out': init_weights([dim_hidden, 3], name='pretrain_out'+str(robot_number)),
            }
            ...
            biases = {
                ...
                'pretrain_b': init_weights([dim_hidden], name='pretrain_b'+str(robot_number)),
                'pretrain_outb': init_weights([3], name='pretrain_outb'+str(robot_number)),
            }
            ...
            pretrain_h = tf.nn.relu(tf.matmul(conv_out_flat, weights['pretrain_w']) + biases['pretrain_b'])
            ...
            nnets.append(TfMap.init_from_lists([nn_input, action, precision], [fc_output, pretrain_target], [loss, pretrain_loss]))
