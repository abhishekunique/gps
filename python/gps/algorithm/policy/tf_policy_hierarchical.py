...
class TfPolicyHierarchical(Policy):
    """
    ...
    """
    def __init__(self, dU, obs_tensor, act_op, feat_op, var, sess, device_string, context_tensor, hierarchy_layer=0, highest_layer=False, copy_param_scope=None):
    ...
    def act(self, x, obs, t, noise, context=None):
        """
        ...
        if self.highest_layer is False:
            context = context.dot(self.scale_context) + self.bias_context
        with tf.device(self.device_string):
            if context is not None:
                action_mean = self.sess.run(self.act_op, feed_dict={self.obs_tensor: obs, self.context_tensor: context})
            else:
